...
export class SctpSignaling {
  ...
  private cookie = ByteBuf.alloc(0);

  private pendingPayloads: ByteBuf[] = [];
  ...
  private validateCookieEcho(cookie: ByteBuf) {
  ...
  private acceptData(tsn: number, flags: number, body: ByteBuf) {
    const payloads: ByteBuf[] = [];
    let currentTsn = tsn;
  ...
  private readDataPayload(tsn: number, flags: number, body: ByteBuf) {
    this.markEstablished();
  ...
  private createDataPacket(payload: ByteBuf) {
    const body = ByteBuf.alloc(12 + payload.length);
  ...
  private createPacket(type: number, flags: number, body: ByteBuf, verificationTag = this.peerTag || 0) {
    const chunk = createSctpChunk(type, flags, body);
    const packet = ByteBuf.alloc(12 + chunk.length);
...
function createSctpChunk(type: number, flags: number, body: ByteBuf) {
  const length = 4 + body.length;
  const paddedLength = align4(length);
  const chunk = ByteBuf.alloc(paddedLength);
  chunk[0] = type;
  chunk[1] = flags;
...
function createSctpParameter(type: number, value: ByteBuf) {
  const length = 4 + value.length;
  const parameter = ByteBuf.alloc(align4(length));
...
export function isSctpPacket(packet: ByteBuf) {
  return packet.length >= 12 &&
    packet.readUInt16BE(0) === SCTP_PORT &&
    packet.readUInt16BE(2) === SCTP_PORT;
}

function hasValidSctpChecksum(packet: ByteBuf) {
